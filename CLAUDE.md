...
### Turn flow
A player takes multiple actions (MOVE, SET_PRODUCTION, SKIP) then emits `END_TURN`. The server advances `currentPlayer`, resets `movesLeft` for all units of the new player, and broadcasts a fresh `stateUpdate`.
