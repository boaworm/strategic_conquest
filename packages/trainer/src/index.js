...
import { genomeToJSON, genomeFromJSON } from './genetics/genome.js';
import { initPopulation, nextGeneration, } from './genetics/population.js';
...
async function main() {
    ...
    if (args.resume) {
        ...
        population = checkpoint.population.map((g) => genomeFromJSON(JSON.stringify(g)));
    ...
    for (let gen = startGen; gen <= generations; gen++) {
        ...
        console.log(`Gen ${String(gen).padStart(4)} | ` +
