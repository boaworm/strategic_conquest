import { UnitType, UnitDomain, UNIT_STATS, Terrain, TileVisibility, wrapX, wrappedDistX, } from '@sc/shared';
import { FEATURE_NAMES } from '../genetics/genome.js';
/**
...
export class EvolvedAgent {
    ...
    scoreMoves(obs, unit) {
        ...
        for (const c of candidates) {
            ...
            if (!tile)
                continue;
            // Domain check
            if (stats.domain === UnitDomain.Land && tile.terrain === Terrain.Ocean)
                continue;
    ...
    // ── Feature extraction ─────────────────────────────────────
    extractMoveFeatures(obs, unit, target) {
        ...
        const nearestFriendly = this.nearestDist(obs.myUnits.filter((u) => u.id !== unit.id), target);
        ...
        for (const adj of this.getAdjacentTiles(target.x, target.y)) {
            ...
            for (const f of obs.myUnits) {
                if (f.x === adj.x && f.y === adj.y && f.id !== unit.id)
                    adjFriendly++;
            }
        }
        ...
        for (const adj of this.getAdjacentTiles(target.x, target.y)) {
            if (adj.y >= 0 && adj.y < this.mapHeight) {
                ...
                if (t && t.visibility === TileVisibility.Hidden)
                    hidden++;
            }
        }
    ...
    extractGlobalFeatures(obs) {
        ...
        const myNaval = obs.myUnits.filter((u) => UNIT_STATS[u.type].domain === UnitDomain.Sea).length;
        ...
        for (const row of obs.tiles) {
            for (const t of row) {
                totalTiles++;
                if (t.visibility === TileVisibility.Visible)
                    visibleCount++;
            }
        }
...
//# sourceMappingURL=evolvedAgent.js.map