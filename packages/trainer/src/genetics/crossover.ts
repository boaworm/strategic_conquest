import { type Genome, GENOME_LENGTH, cloneGenome } from './genome.js';

/**
 * Single-point crossover: pick a random cut point and swap tails.
 */
export function crossover(parent1: Genome, parent2: Genome, rng: () => number): [Genome, Genome] {
 ...
 */
export function uniformCrossover(parent1: Genome, parent2: Genome, rng: () => number): Genome {
 ...
 */
export function mutate(genome: Genome, rate: number, strength: number, rng: () => number): void {
  for (let i = 0; i < GENOME_LENGTH; i++) {
    if (rng() < rate) {
