import { type Genome, randomGenome, cloneGenome } from './genome.js';
import { crossover, mutate } from './crossover.js';

export interface PopulationConfig {
  size: number;
  eliteCount: number;       // # of top agents that survive unchanged
  mutationRate: number;      // per-gene mutation probability (0–1)
  mutationStrength: number;  // Gaussian noise σ
  seed?: number;
}
 ...
 */
export function initPopulation(config: PopulationConfig): Genome[] {
 ...
 */
function tournamentSelect(ranked: RankedAgent[], k: number, rng: () => number): Genome {
