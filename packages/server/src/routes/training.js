...
export function createTrainingRoutes(manager) {
    const router = Router();
    // ── Game Creation ────────────────────────────────────────────
    /**
    ...
    router.post('/training/games', (req, res) => {
        ...
        const session = manager.createGame(mapWidth ?? 30, mapHeight ?? 20, true, // isPvE
        difficulty);
        const gameInfo = {
            gameId: session.id,
    ...
    router.post('/training/games/:gameId/step', (req, res) => {
        ...
        const done = session.state.phase === GamePhase.Finished;
        res.json({
            success: true,
            reward,
            done,
     ...
     */
    function calculateReward(stateBefore, stateAfter, playerId) {
        let reward = 0;
        // City differential
        const citiesBefore = stateBefore.cities.filter((c) => c.owner === playerId).length;
        const citiesAfter = stateAfter.cities.filter((c) => c.owner === playerId).length;
        ...
        const unitsBefore = stateBefore.units.filter((u) => u.owner === playerId).length;
        const unitsAfter = stateAfter.units.filter((u) => u.owner === playerId).length;
