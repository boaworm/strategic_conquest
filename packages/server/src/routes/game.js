...
export function createGameRoutes(manager) {
    const router = Router();
    /**
     * POST /api/games
    ...
    router.post('/games', (req, res) => {
        ...
        const diff = (isPvE || isAiVsAi) ? (difficulty ?? 'medium') : 'medium';
        const session = manager.createGame(mapWidth ?? 60, mapHeight ?? 40, isPvE, diff, p1Type ?? 'human', p2Type ?? 'human');
        const response = {
            gameId: session.id,
