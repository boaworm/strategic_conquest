...
export default function App() {
    ...
    const [cityDialogId, setCityDialogId] = useState(null);
    ...
    if (!connected || !view) {
        ...
        <MainMenu />
        {connected && !view && (<div className="text-center text-gray-400 mt-8">
            Waiting for opponent to join...
          </div>)}
      </div>);
    }
    return (<div className="h-screen bg-gray-950 flex flex-col overflow-hidden" onKeyDown={(e) => {
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden relative">
          <GameCanvas view={view} onCityClick={(city) => setCityDialogId(city.id)} selectedCityId={cityDialogId}/>
        </div>
        ...
        <div className="w-60 p-2 space-y-2 overflow-y-auto">
          ...
          <div className="bg-gray-800 text-white p-3 rounded space-y-1">
            ...
            {view.myCities.map((city) => (<button key={city.id} className={`w-full text-left text-xs px-2 py-1 rounded hover:bg-gray-700 ${cityDialogId === city.id ? 'bg-purple-900 ring-1 ring-purple-500' : 'bg-gray-900'}`} onClick={() => setCityDialogId(city.id)}>
                ({city.x},{city.y}){' '}
                {city.producing ? (<span className="capitalize">
                    ({city.productionTurnsLeft}) {city.producing}
                  </span>) : (<span className="text-gray-500">(0) Idle</span>)}
              </button>))}
          </div>

          {/* City production dialog — inline in sidebar */}
          {cityDialog && (<CityDialog cityId={cityDialog.id} currentProduction={cityDialog.producing} turnsLeft={cityDialog.productionTurnsLeft} coastal={cityDialog.coastal} onClose={() => setCityDialogId(null)}/>)}
        </div>
      </div>

      {/* Win / Loss popup */}
      {view.phase === GamePhase.Finished && view.winner && (<div className="fixed inset-0 flex items-center justify-center z-50 bg-black/60">
