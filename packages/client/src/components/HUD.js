...
export function HUD() {
    ...
    return (<div className="bg-gray-900 text-white px-4 py-2 flex items-center justify-between gap-4 text-sm">
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        ...
        {view.phase === GamePhase.Finished && view.winner && (<span className={view.winner === playerId ? 'text-green-400 font-bold' : 'text-red-400 font-bold'}>
        ...
        <label className="flex items-center gap-1 text-xs text-gray-300 cursor-pointer select-none">
          <input type="checkbox" checked={autoSelectNext} onChange={(e) => setAutoSelectNext(e.target.checked)} className="accent-blue-500"/>
          Select next unit
        </label>
        <label className="flex items-center gap-1 text-xs text-gray-300 cursor-pointer select-none">
          <input type="checkbox" checked={autoEndTurn} onChange={(e) => setAutoEndTurn(e.target.checked)} className="accent-blue-500"/>
          Auto end turn
        </label>
        {isMyTurn && view.phase === GamePhase.Active && (<button className="px-3 py-1 bg-blue-700 rounded hover:bg-blue-600" onClick={() => sendAction({ type: 'END_TURN' })}>
            End Turn
          </button>)}
      </div>
    </div>);
