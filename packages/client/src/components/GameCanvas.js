...
import { TileVisibility, Terrain, UnitType, UnitDomain, UNIT_STATS, wrapX, wrappedDistX, } from '@sc/shared';
...
function drawUnitShape(ctx, type, cx, cy, size, color) {
    ctx.save();
    const r = size * 0.38;
    ctx.fillStyle = color;
...
function drawIceCap(ctx, x, y, size, south) {
    // White base
...
export function GameCanvas({ view, onCityClick, selectedCityId }) {
    ...
    const draw = useCallback(() => {
        ...
        if (endTileX - startTileX > 2000 || endTileY - startTileY > 2000)
            return;
        // Clear
        ...
        for (const u of cachedAllUnits) {
            ...
            if (anim && anim.phase !== 'done' && u.id === anim.attackerUnitId)
                continue;
            // During move animation, skip the unit
            const mAnim = moveAnimRef.current;
            if (mAnim && u.id === mAnim.unitId)
                continue;
            const key = `${u.x},${u.y}`;
        ...
        for (let wy = startTileY; wy <= endTileY; wy++) {
            for (let wx = startTileX; wx <= endTileX; wx++) {
                ...
                if (units && units.length > 0) {
                    ...
                    if ((unit.type === UnitType.Carrier || unit.type === UnitType.Transport) &&
                        unit.cargo.length > 0 &&
                        tileSize >= 14) {
                        ctx.fillStyle = '#fff';
                    ...
                    if ((unit.type === UnitType.Battleship || unit.type === UnitType.Carrier) &&
    ...
    const requestRedraw = useCallback(() => {
        if (redrawRequestedRef.current !== null)
            return;
    ...
    useEffect(() => {
        return () => {
            if (redrawRequestedRef.current !== null)
                cancelAnimationFrame(redrawRequestedRef.current);
        };
    }, []);
    ...
    function handleMouseUp(e) {
        ...
        if (e.shiftKey) {
            const myUnitsHere = view.myUnits.filter((u) => u.x === tx && u.y === ty && !u.carriedBy);
        ...
        if (selectedUnitId) {
            ...
            if (unit) {
                ...
                if (unit.movesLeft > 0 && !unit.carriedBy) {
                    if (dx <= 1 && dy <= 1) {
                        const enemyOnTarget = view.visibleEnemyUnits.find((e) => e.x === tx && e.y === ty && !e.carriedBy);
                        if (enemyOnTarget) {
                    ...
                    if (path.length > 0) {
                        ...
                        const enemyOnLast = view.visibleEnemyUnits.find((e) => e.x === lastStep.x && e.y === lastStep.y && !e.carriedBy);
                        if (!reachedTarget && !enemyOnLast)
                            return;
                        if (enemyOnLast) {
     ...
     */
    function computePath(fx, fy, goalX, goalY, maxSteps, domain) {
        const path = [];
        let cx = fx, cy = fy;
        for (let i = 0; i < maxSteps; i++) {
            ...
            const candidates = [];
            if (stepX !== 0 && stepY !== 0)
                candidates.push({ x: cx + stepX, y: cy + stepY });
            if (stepX !== 0)
                candidates.push({ x: cx + stepX, y: cy });
            if (stepY !== 0)
                candidates.push({ x: cx, y: cy + stepY });
            for (const dx of [-1, 0, 1]) {
                for (const dy of [-1, 0, 1]) {
                    if (dx === 0 && dy === 0)
                        continue;
                    const c = { x: cx + dx, y: cy + dy };
                    if (!candidates.some((p) => p.x === c.x && p.y === c.y))
                        candidates.push(c);
                }
            }
            let moved = false;
            for (const c of candidates) {
                ...
                if (terrain === undefined)
                    continue;
                if (domain === UnitDomain.Land && terrain === Terrain.Ocean)
                    continue;
                if (domain === UnitDomain.Sea && terrain === Terrain.Land) {
                    const hasCity = [...view.myCities, ...view.visibleEnemyCities].some((ci) => ci.x === nx && ci.y === ny);
                    if (!hasCity)
                        continue;
                }
                const enemyHere = view.visibleEnemyUnits.some((u) => u.x === nx && u.y === ny && !u.carriedBy);
                if (enemyHere) {
                    path.push({ x: nx, y: ny });
                    moved = true;
                    break;
            ...
            if (path.length > 0) {
                ...
                const enemyOnLast = view.visibleEnemyUnits.some((u) => u.x === last.x && u.y === last.y && !u.carriedBy);
                if (enemyOnLast)
                    break;
            }
        }
    ...
    return (<canvas ref={canvasRef} tabIndex={0} onMouseUp={handleMouseUp} onContextMenu={(e) => e.preventDefault()} onKeyDown={(e) => {
            if (e.key === ' ' && selectedUnitId) {
                e.preventDefault();
                sendAction({ type: 'SKIP', unitId: selectedUnitId });
            }
        }} className="w-full h-full block cursor-grab active:cursor-grabbing outline-none touch-none" style={{ imageRendering: 'pixelated' }}/>);
}
