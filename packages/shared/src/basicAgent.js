...
export class BasicAgent {
    ...
    decideSeaUnit(obs, unit) {
        ...
        if (stats.cargoCapacity > 0 && unit.type === UnitType.Transport) {
            ...
            const armyToLoad = obs.myUnits.find((u) => u.type === UnitType.Infantry &&
                u.carriedBy === null &&
        ...
        const enemyShip = this.nearestEnemy(obs.visibleEnemyUnits.filter((u) => UNIT_STATS[u.type].domain === UnitDomain.Sea), unit);
        if (enemyShip) {
