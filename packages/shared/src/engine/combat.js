 ...
 */
export function resolveCombat(state, attacker, defender, noRetaliation = false) {
    const aStats = UNIT_STATS[attacker.type];
    const dStats = UNIT_STATS[defender.type];
    // Attacker rolls
    const attackerHits = d6() <= aStats.attack;
    // Defender rolls (suppressed for submarine attacker or shore bombardment)
    const defenderHits = noRetaliation || attacker.type === UnitType.Submarine
        ? false
        : d6() <= dStats.defense;
    let attackerDmg = 0;
    let defenderDmg = 0;
 ...
 */
export function removeDestroyedUnits(state) {
    const removedIds = [];
