import { Terrain, UnitDomain, UnitType, UNIT_STATS, wrapX, wrappedDistX, } from '../types.js';
/**
 ...
 */
export function distToNearestFriendlyCity(state, x, y, owner) {
    let best = Infinity;
 ...
 */
export function distToNearestLandingSpot(state, x, y, owner, unitType) {
    let best = Infinity;
    // Friendly cities
    for (const city of state.cities) {
        if (city.owner !== owner)
    ...
    if (unitType === UnitType.Fighter) {
        for (const u of state.units) {
            if (u.owner !== owner)
                continue;
            if (u.type !== UnitType.Carrier)
                continue;
            if (u.carriedBy !== null)
                continue;
            if (u.cargo.length >= UNIT_STATS[u.type].cargoCapacity)
                continue;
 ...
 */
export function canMoveTo(state, unit, to) {
    // Wrap X
 ...
 */
export function normalizeCoord(to, mapWidth) {
    return { x: wrapX(to.x, mapWidth), y: to.y };
}
/**
 * Get all friendly units on a specific tile.
 */
export function getUnitsAt(state, pos, owner) {
    const wx = wrapX(pos.x, state.mapWidth);
    return state.units.filter((u) => u.x === wx &&
        u.y === pos.y &&
        u.carriedBy === null &&
 ...
 */
export function getVisibleTiles(state, playerId) {
 ...
 */
export function canDetectSubmarine(state, x, y, playerId) {
    ...
    for (const unit of state.units) {
        ...
        if (unit.carriedBy !== null)
            continue;
        if (unit.type !== UnitType.Destroyer && unit.type !== UnitType.Submarine)
            continue;
        const stats = UNIT_STATS[unit.type];
        const dx = wrappedDistX(wx, unit.x, state.mapWidth);
        const dy = Math.abs(y - unit.y);
        if (dx <= stats.vision && dy <= stats.vision)
            return true;
    }
    return false;
}
