import { GamePhase, TileVisibility, Terrain, UnitDomain, UnitType, UNIT_STATS, wrapX, wrappedDistX, } from '../types.js';
...
function handleMove(state, unitId, to, playerId) {
    ...
    if (unit.carriedBy !== null)
        return { success: false, error: 'Unit is being carried' };
    ...
    if (unit.type === UnitType.Battleship) {
        ...
        if (terrain !== undefined && terrain !== Terrain.Ocean) {
            const enemyLand = getUnitsAt(state, target).filter((u) => u.owner !== playerId && UNIT_STATS[u.type].domain === UnitDomain.Land);
    ...
    if (UNIT_STATS[unit.type].domain === UnitDomain.Land &&
        state.tiles[target.y]?.[target.x] === Terrain.Ocean &&
        unit.movesLeft > 0) {
        const transport = state.units.find((u) => u.x === target.x &&
            u.y === target.y &&
            u.owner === playerId &&
            u.carriedBy === null &&
    ...
    const enemyUnits = allEnemyUnits.filter((u) => u.type !== UnitType.Submarine || subDetected);
    if (enemyUnits.length > 0) {
        ...
        if (unit.type === UnitType.Submarine) {
            const hasNavalTarget = enemyUnits.some((e) => UNIT_STATS[e.type].domain === UnitDomain.Sea);
            if (!hasNavalTarget) {
        ...
        if (unit.type === UnitType.Bomber) {
            ...
            for (const pos of affectedTiles) {
                const unitsOnTile = state.units.filter((u) => u.x === pos.x && u.y === pos.y && u.carriedBy === null && u.id !== unit.id);
                for (const u of unitsOnTile) {
                    // Radius 0: only enemies. Radius 1+: everyone (friendly and enemy)
                    if (blastRadius === 0 && u.owner === playerId)
                        continue;
                    u.health = 0;
                }
            }
    ...
    if (unit.type === UnitType.Fighter) {
        const carrier = state.units.find((u) => u.x === unit.x &&
            u.y === unit.y &&
            u.owner === playerId &&
    ...
    if (unit.type === UnitType.Fighter || unit.type === UnitType.Bomber) {
        const onFriendlyCity = state.cities.some((c) => c.x === unit.x && c.y === unit.y && c.owner === playerId);
        if (onFriendlyCity) {
            unit.movesLeft = 0;
            const stats = UNIT_STATS[unit.type];
            if (stats.maxFuel !== undefined)
...
function handleSkip(state, unitId, playerId) {
    const unit = state.units.find((u) => u.id === unitId);
    if (!unit)
        return { success: false, error: 'Unit not found' };
    if (unit.owner !== playerId)
...
function handleDisband(state, unitId, playerId) {
    const unit = state.units.find((u) => u.id === unitId);
    if (!unit)
        return { success: false, error: 'Unit not found' };
    if (unit.owner !== playerId)
...
function tryCaptureCity(state, unit, playerId) {
    if (unit.type !== 'infantry' && unit.type !== 'tank')
        return null;
 ...
 */
export function isCityCoastal(state, city) {
...
function handleSetProduction(state, cityId, unitType, playerId) {
    const city = state.cities.find((c) => c.id === cityId);
    if (!city)
        return { success: false, error: 'City not found' };
    if (city.owner !== playerId)
        return { success: false, error: 'Not your city' };
...
function handleSleep(state, unitId, playerId, sleep) {
    const unit = state.units.find((u) => u.id === unitId);
    if (!unit)
        return { success: false, error: 'Unit not found' };
    if (unit.owner !== playerId)
...
function handleLoad(state, unitId, transportId, playerId) {
    const unit = state.units.find((u) => u.id === unitId);
...
function handleUnload(state, unitId, to, playerId) {
    const unit = state.units.find((u) => u.id === unitId);
    if (!unit)
        return { success: false, error: 'Unit not found' };
    if (unit.owner !== playerId)
        return { success: false, error: 'Not your unit' };
    if (!unit.carriedBy)
        return { success: false, error: 'Unit is not being carried' };
    if (unit.movesLeft <= 0)
        return { success: false, error: 'No moves left' };
    const transport = state.units.find((u) => u.id === unit.carriedBy);
    if (!transport)
...
function handleEndTurn(state, playerId) {
    ...
    for (const unit of state.units) {
        ...
        if (unit.type !== UnitType.Fighter)
            continue;
        if (unit.carriedBy !== null)
            continue; // safe on carrier
        const onCity = state.cities.some((c) => c.x === unit.x && c.y === unit.y && c.owner === unit.owner);
        if (!onCity) {
            unit.health = 0;
            fightersCrashed++;
        }
...
function getBomberBlastRadius(state, playerId) {
...
function getTilesInRadius(cx, cy, radius, mapWidth, mapHeight) {
    const tiles = [];
...
export function getPlayerView(state, playerId) {
    ...
    // Build tile view
    const tiles = Array.from({ length: state.mapHeight }, (_, y) => Array.from({ length: state.mapWidth }, (_, x) => {
        const key = `${x},${y}`;
    ...
    const currentlyVisible = state.units
        .filter((u) => u.owner !== playerId &&
        u.carriedBy === null &&
        visible.has(`${u.x},${u.y}`) &&
    ...
    state.seenEnemies[playerId] = seen.filter((s) => state.units.some((u) => u.id === s.id));
    ...
    const ghostEnemies = state.seenEnemies[playerId]
    ...
    const visibleEnemyCities = state.cities
